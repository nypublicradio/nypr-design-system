...
Tooltips are floating pieces of contextual information about an element on screen that help a user to better understand something onsite. They are displayed when a user hovers or focuses on the element it is describing. While toolstips should typically be attached to icons, they can also be triggered by links, text, buttons, or inputs. If the information is critical for the user to achieve a task, keep the information clear onsite rather than hiding within a tooltip.
...
Though they can look visually similar, a drawer menu is different from a tooltip due to the function it provides to a user. At its core, a drawer menu is a menu, used to give a user access to extended actionable, interactive options.  
...
Tooltips should always be positioned next to the element they are connected to. Icon tooltips and interactive tooltips may be positioned top, bottom, left, or right to the trigger item. The position will vary depending on where the element is in relation to the edge of the screen.
