...
Use modifier classes to provide styling options for components. The M in BEM, `--modifier`, lets you provide
...
[Higher order components]() is a powerful pattern that let you build components that can be invoked in a very flexible way. It's handy when you want components that can handle a wide variety of use cases. 
...
```html
/* Ideal form of the most common use case */
...
Radial is designed to serve our content. So it should be easy to use components with our content straight out of the API. Components should accept objects straight from our most common APIs. Our APIs may change over time, but right now articles should match Wagtail articles. This makes components easier to use in our clients and also makes it easier to handle list components.
...
// An ember helper that maps one object to another
```
